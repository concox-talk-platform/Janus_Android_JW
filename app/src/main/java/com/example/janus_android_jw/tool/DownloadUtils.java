...
public class DownloadUtils  implements TextToSpeech.OnInitListener{
    //下载器
                ...
                case DownloadManager.STATUS_FAILED:
                    ...
                    Thread myThread=new Thread(){
                        ...
                        public void run() {
                            try{
                                sleep(100);
                                String text = "Download apk failed";
    ...
    public void onInit(int status) {
        if (status == TextToSpeech.SUCCESS) {
            ...
            if (result == TextToSpeech.LANG_MISSING_DATA || result == TextToSpeech.LANG_NOT_SUPPORTED) {
                Toast.makeText(mContext, "no support", Toast.LENGTH_SHORT).show();
            } else {
                //不支持中文就将语言设置为英文
